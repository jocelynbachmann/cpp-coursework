...
void AVLTree<K, V>::rebalance(Node *&subtree)
{
    ...
    if (balance > 1)
    {
        height(subtree->left->left) > height(subtree->left->right) ? rotateRight(subtree) : rotateLeftRight(subtree);
    }
    else if (balance < -1)
    {
        height(subtree->right->right) > height(subtree->right->left) ? rotateLeft(subtree) : rotateRightLeft(subtree);
    }
    else
...
void AVLTree<K, V>::remove(Node *&subtree, const K &key)
{
    ...
    else
    {
        ...
        else if (subtree->left != NULL && subtree->right != NULL)
        {
            ...
            // if parent is subtree then predecessor is parent's left
            // otherwise it's parent's right
            parent == subtree ? parent->left = pred->left : parent->right = pred->left;
