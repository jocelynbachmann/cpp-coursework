...
HSLAPixel* myFavoriteColor(double saturation) {
    // the stack memory local variable bug
    // BUG 2
    // CHANGED: makes new HSLAPixel on heap, instead of returning address of stack variable
...
void sketchify(std::string inputFile, std::string outputFile) {
    // Load in.png
    // BUG 1
    // CHANGED: initialized original as a new PNG
    ...
    // MORE DEBUGGING!
    // CHANGED: loop stops one earlier to avoid out of bounds
    for (unsigned y = 1; y < height - 1; y++) {
        for (unsigned x = 1; x < width - 1; x++) {
            ...
            if (score > 0.3) {
                // MORE DEBUGGING!
                // CHANGED: using dereference to change the values and not the pointers
