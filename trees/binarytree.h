...
struct Node {
    ...
    Node* root; // NOTE: exposing root for flattree
  ...
  private:
    ...
    * @param upper
    */
    bool isOrdered(const Node* subRoot, T lower, T upper) const;

    bool isOrdered2(const Node* subRoot, const Node* left, const Node* right) const;
    ...
    * @param subRoot
    */
    void vectorize(const Node* subRoot, vector<T>& result) const;
